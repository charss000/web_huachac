jQuery(document).ready(function($) {
	...
	jQuery('#cff_photos_source').on('change', function(){
	...
	jQuery('#cff_albums_source').on('change', function(){
	...
	jQuery('#cff_videos_source').on('change', function(){
	...
	jQuery('#cff_events_source').on('change', function(){
	...
	jQuery("#post-types").on('change', function() {
	...
	jQuery("#cff_show_others").on('change', function() {
	...
	jQuery('.cff-layout').on('click', function(){
        var $self = jQuery(this);
  ...
  jQuery('#cff-admin th').on('mouseenter', function(){
  ...
  jQuery('#cff-admin label').on('mouseenter', function(){
  ...
  //Caching options
	...
	$('input[type=radio][name=cff_caching_type]').on('change', function() {
      if (this.value == 'page') {
  ...
  //Should we show the caching time settings?
	...
	jQuery('#cff_cache_cron_interval').on('change', function(){
	...
	if ($('.cff-page-selected').length) {
		$('#cff-insert-token, .cff-insert-reviews-token, .cff-insert-both-tokens').attr('disabled',false);
	}
	$('.cff-managed-page').on('click', function(){
		...
		var groupNotice = false, $groupNoticeHTML = $('.cff-note-group-notice');
		$('.cff-managed-page[data-pagetype="group"]').each(function(){
			if($(this).hasClass('cff-page-selected'))
				groupNotice = true;
		});
		if(groupNotice){
			$groupNoticeHTML.show();
		}else{
			$groupNoticeHTML.hide();
		}
		if ($('.cff-page-selected').length) {
			$('#cff-insert-token, .cff-insert-reviews-token, .cff-insert-both-tokens').attr('disabled',false);
		} else {
			$('#cff-insert-token, .cff-insert-reviews-token, .cff-insert-both-tokens').attr('disabled',true);
		}
	...
	if( cff_connected_accounts_val !== '' && cff_connected_accounts_val !== '{}' && typeof cff_connected_accounts_val !== 'undefined' ){
		...
		cff_connected_accounts = cff_connected_accounts_val.replace(/\\"/g, '"').replace('loadcommentsjs=true','');
	...
	$('#cff-insert-token, #cff-insert-all-tokens').on('click', function(){
		...
		//Add token to setting if it's emprty or needs to be replaced (ie. it results in an API error)
	...
	$('#cff_manual_account_step_2 input[type=submit]').on('click', function(e){
		...
		addConnectedAccounts(
			$cff_manual_account.find('#cff_manual_account_id').val().replace(/"/g, ''),
			$cff_manual_account.find('#cff_manual_account_name').val().replace(/"/g, ''),
			$cff_manual_account.find('#cff_manual_account_type').val(),
			$cff_manual_account.find('#cff_manual_account_token').val().replace(/\W/g, ''),
			false
		);
	});
	...
	jQuery('#cff_masonry_desktop_col').on('change', function(){
	...
	jQuery('#cff-group-app-tooltip').on('mouseenter', function(){
	    jQuery('#cff-group-app-screenshot, .cff-group-app-screenshot').show();
	}).on('mouseleave',function(){
		jQuery('#cff-group-app-screenshot, .cff-group-app-screenshot').hide();
	});
	...
	// clear resized
	var $cffClearResizedButton = $('#cff_reset_resized');

	$cffClearResizedButton.on('click', function(event) {
		event.preventDefault();
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(data=='1') {
					$cffClearResizedButton.after('<i id="cff-clear-cache-success" class="fa fa-check-circle cff-success"></i>');
				} else {
					$cffClearResizedButton.after('<span>error</span>');
				}
			}
		}); // ajax call
	...
	$(document).on('click', "#recheck-license-key", function() {
		...
		let cffLicenseNotice = $('#cff-license-notice');
		$.ajax({
			url: ajaxurl,
			data: {
			...
			success: function(result){
				...
				if ( cffLicenseNotice ) {
					if ( result.success == true ) {
						cffLicenseNotice.removeClass('cff-license-expired-notice').addClass('cff-license-renewed-notice');
					}
	...
	$(document).on('click', "#cff-hide-notice", function() {
		let cffLicenseNotice = $('#cff-license-notice');
		let cffLicenseModal = $('.cff-sb-modal');
