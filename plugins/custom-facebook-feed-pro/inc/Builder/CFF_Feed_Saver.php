...
class CFF_Feed_Saver {
	...
	public function get_feed_settings() {
		...
		if ( is_array( $return['sources'] ) ) {
			$ids_in_feed = [];
			foreach ( $return['sources'] as $source ) {
				if ( is_string( $source ) ) {
					$ids_in_feed[] = $source;
				} else {
					$ids_in_feed[] = $source['account_id'];
				}
			}
			$args = array( 'id' => $ids_in_feed );
		} else {
		...
		if ( ! empty( $source_query ) ) {
			...
			if ( ! empty( $found_sources ) ) {
				...
				$return['accesstoken'] = stripslashes( $source[0]['access_token'] );
				$return['id'] = stripslashes( $source[0]['account_id'] );
			} else {
	...
	public static function settings_defaults( $return_array = true ) {
		{
			...
			$defaults = array(
				...
				'oneimage' => '',
				'mediaposition' => 'below',
				'include' => 'author,text,desc,sharedlinks,date,media,medialink,eventtitle,eventdetails,social,link,likebox',
				'exclude' => '',
				'masonry' => '',
				'masonrycols' => '',
				'masonrycolsmobile' => '',
